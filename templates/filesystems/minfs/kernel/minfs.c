...
static int minfs_readdir(struct file *filp, struct dir_context *ctx)
{
	...
	for (; ctx->pos < MINFS_NUM_ENTRIES; ctx->pos++) {
		...
		/* Step over empty entries in dentry list(de->ino == 0). */
		if (de->ino == 0) {
			continue;
		}
		...
		over = dir_emit(ctx, de->name, MINFS_NAME_LEN, de->ino,
				DT_UNKNOWN);
		/*when dir_emit() return not zero，buffer in userspace is fulled */
		if (over) {
...
/*
 * Find dentry in parent folder. Return parent folder's data buffer_head(struct minfs_dir_entry).
 */
		...
		struct dentry *dentry, unsigned int flags)
{
	...
	/* Associating an inode with a dentry*/
	d_add(dentry, inode);
	brelse(bh);
...
static struct inode *minfs_new_inode(struct inode *dir)
{
	...
	/* add inode to inode hashtable,if inode is dirty, this inode will be write to disk*/
...
static int minfs_add_link(struct dentry *dentry, struct inode *inode)
{
	...
	/* Get: directory inode (in inode); containing inode (in mii); superblock (in sb). */
