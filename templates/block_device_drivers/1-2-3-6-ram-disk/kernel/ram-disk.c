...
{
	/* iterate segments in request*/
	struct bio_vec bvec;
	struct req_iterator iter;

	rq_for_each_segment(bvec, req, iter) {
		sector_t sector = iter.iter.bi_sector;//段迭代器获取当前扇区
		unsigned long offset = bvec.bv_offset;
		size_t len = bvec.bv_len;
		int dir = bio_data_dir(iter.bio);//请求迭代器获取对当前 struct bio 的引用
				     ...
				     const struct blk_mq_queue_data *bd)
{
	...
	/* check fs request. Return if passthrough. */
...
static int create_block_device(struct my_block_dev *dev)
{
	...
	/* set device sector size to queue */
